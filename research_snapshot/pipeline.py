...
def _collect_source(source: dict[str, Any], raw_dir: Path) -> Path:
    source_type = source.get("type")
    location = source.get("location")
    if source_type not in {"local_csv", "local_json", "http_csv", "http_json"}:
...
def build_snapshot(request_path: Path | str, run_dir: Path | str) -> dict[str, Any]:
    ...
    for source in request["sources"]:
        raw_path = _collect_source(source, raw_dir)
