...
## 첫 화면 의사결정 블록 — 다른 패널보다 먼저 (반드시 별도 박스로 시각화)

첫 화면에서 목표지수나 시나리오 밴드보다 다음 네 항목을 먼저 보여준다. **서술형 문단에 녹여 쓰는 것으로는 이 요건을 충족하지 않는다** — 증권사 리포트가 커버페이지에 투자의견·목표주가·핵심 지표를 별도 박스로 분리해 스크롤 없이 훑을 수 있게 하듯, 본문과 시각적으로 구분되는 독립 박스(카드/패널)여야 한다.
...
본문 표에 흩어진 핵심 수치를 한곳에 모은 참조용 표를 본문 하단(또는 별도 부록 섹션)에 둔다 — 증권사 리포트의 재무제표 부록과 같은 역할이다. 최소한 대표 관측일(과거 위기 저점들 + 이번 저점 + 직후 관측일)의 원자료 값과 "과거 대비 위치(백분위)"를 담는다. 본문에서 이미 설명한 숫자를 재복사하는 것이 아니라, 검증·재확인이 필요할 때 한 번에 대조할 수 있는 표로 존재해야 한다.

## 콜로폰(발행 정보) 섹션 — 필수

마무리 고지와 별도로, 다음 세 가지를 짧게 담은 콜로폰 블록을 둔다 — 증권사 리포트의 컴플라이언스 공시(애널리스트 이해관계·제3자 제공 여부)에 대응한다.

1. **이 글이 어떻게 만들어졌는가**: 다수 에이전트가 리서치→시나리오→사후심사 세 단계를 거쳤고, 하나의 결론으로 미리 수렴시키지 않았다는 점
2. **이해관계 고지**: 분석 주체가 코스피·개별 종목에 어떤 포지션도 보유하지 않으며, 특정 매매를 유도할 유인이 없다는 점 (목표주가·매수의견처럼 결론을 하나의 숫자로 좁히는 요소를 의도적으로 배제했다는 점 포함)
3. **한계**: raw 부재 시 공개 보고서로 보완했다는 점과 1회성 정적 분석이라 자동 갱신되지 않는다는 점

이 세 섹션(첫 화면 의사결정 블록, 원자료 관측값 부록, 콜로폰)은 "증권사 프로세스를 모티브로 한 리포트 완성도" 기준의 핵심이며, 목표주가·매수의견 같은 투자 조언 요소를 넣지 않고도 형식적 완성도를 높이기 위해 도입했다(2026-08-05 결정, CLAUDE.md 변경이력 참조).
