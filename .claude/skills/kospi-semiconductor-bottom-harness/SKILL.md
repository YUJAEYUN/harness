...
3. 존재 → 사용자에게 "이전 분석 결과가 있습니다. 새로 분석할까요, 아니면 특정 부분만 다시 하시겠어요?"라고 묻는다. 새 분석이면 기존 `_workspace/`를 `_workspace_{YYYYMMDD_HHMMSS}/`로 이동 후 Phase 1. 부분 수정이면 해당 에이전트만 개별 Agent 호출로 재작업(팀 전체를 다시 만들지 않음)
...
3. 팀원들이 자체 조율하며 조사 수행, 완료 시 `_workspace/01_{agent}_report.md` 저장
4. research-team-lead가 검수 후 `_workspace/01_research_synthesis.md` + `_workspace/01_research_escalations.md` 작성
5. PM은 `_workspace/01_research_escalations.md`를 읽어두되, **즉시 사용자에게 묻지 않는다** — 단, 분석의 전제 자체(고점/저점 정의 등 4장 시나리오 분기의 기준)를 흔드는 항목이면 예외적으로 이 시점에 바로 확인
6. `TeamDelete`로 research-team 정리

### Phase 3: 웨이브 2 — 시나리오팀 (에이전트 팀)

1. `TeamCreate(team_name: "scenario-team", members: [scenario-team-lead + 6 roles])` — `_workspace/01_research_synthesis.md`를 공통 입력 경로로 전달
...
4. scenario-team-lead가 검수 후 `_workspace/02_scenario_synthesis.md` + `_workspace/02_scenario_escalations.md` 작성
5. `TeamDelete`로 scenario-team 정리

### Phase 4: PM 통합

1. 리서치팀·시나리오팀 종합본 정합성 확인 — 시나리오팀이 리서치팀 사실관계를 왜곡 없이 인용했는지 대조
2. `_workspace/01_research_escalations.md` + `_workspace/02_scenario_escalations.md`를 취합해 **한 번에** 사용자에게 확인 (AskUserQuestion 등으로 일괄 질문 — 여러 번 나눠 묻지 않는다)
...
1. `output/kospi-semiconductor-bottom-dashboard.html` — 인터랙티브(클라이언트 사이드 UI만, 데이터는 정적) 대시보드
2. `output/미확인_가정.md` — 확인 없이 진행한 모든 항목 (팀 내 조정 로그 + 사소해서 사용자에게 안 올린 항목 전부)
3. 두 문서 모두 "투자 조언이 아님" 명시 (대시보드는 눈에 띄는 위치에 고정 배너)

### Phase 6: 정리 및 피드백

1. `_workspace/` 보존 (삭제하지 않음 — 사후 검증·추적용)
...
7. 예상 결과: `output/kospi-semiconductor-bottom-dashboard.html`, `output/미확인_가정.md` 생성

### 에러 흐름
1. Phase 2에서 geopolitical-analyst가 응답 없이 중지
2. research-team-lead가 1회 재작업 지시 → 재실패
3. `_workspace/01_research_synthesis.md`에 "지정학 섹션 일부 미수집" 명시하고 나머지로 진행
