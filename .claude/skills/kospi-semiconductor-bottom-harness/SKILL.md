...
#### Phase 5-1: 결정적 신뢰도 게이트 — 필수

문서 체크리스트만으로는 원자료 변조, 날짜 불일치, 폐기한 숫자의 회귀를 안정적으로 차단할 수 없다. Phase 6 진입 전에 반드시 번들 검증기를 실행하라.

```bash
python3 .claude/skills/kospi-semiconductor-bottom-harness/scripts/validate_run.py \
  --report _workspace/kospi-bottom/validation_report.json
```

- 계약 파일: `references/run-contract.json`
- 검증 범위: 원자료 SHA-256, 필수 컬럼, 행 수, 날짜 정렬·중복·정렬, 대표 관측치, PER·PBR 분포·백분위, 최신 IC 등급, 필수 고지·미확인 항목
- 결과가 `FAIL`이거나 프로세스 종료 코드가 0이 아니면 **Phase 6로 진행하지 말고** 실패 항목을 수정한 뒤 재실행하라.
- 새 원자료로 기준일을 갱신할 때는 검증을 우회하지 말고 `run-contract.json`의 기준일·해시·대표값을 같은 리뷰 단위에서 갱신하라.
...
### 자동 검증기 회귀 테스트
1. `python3 -m unittest discover -s tests -p 'test_*.py' -v`를 실행한다.
